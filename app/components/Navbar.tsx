...
import WellMindFloatingWidget from './WellMindFloatingWidget'

export default function Navbar() {
  const [scrolled, setScrolled] = useState(false)
  const [logoSrc, setLogoSrc] = useState('/api/logo')
  const [showWatsonAgent, setShowWatsonAgent] = useState(false)
    ...
    <header className={`fixed top-0 inset-x-0 z-40 transition-all ${scrolled ? 'backdrop-blur bg-white/80 shadow-card' : 'bg-white/90'} border-b border-slate-200`}>
      <nav className="max-w-7xl mx-auto flex items-center justify-between px-6 py-2.5">
        <Link href="/" className="flex items-center gap-2 group">
          <img src="/Wellmind-LOGO.jpeg" alt="vorteX logo" onError={handleLogoError} className="h-8 w-auto rounded-md shadow-card ml-2" />
        ...
        <div className="flex items-center gap-4">
          ...
          <button onClick={() => setShowWatsonAgent(true)} className="btn btn-care text-sm">Care</button>
        </div>
      </nav>
      <WellMindFloatingWidget open={showWatsonAgent} onClose={() => setShowWatsonAgent(false)} />
    </header>
  )
}